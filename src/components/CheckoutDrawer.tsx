                  ...
                  <div className="space-y-2">{shippingOptions.map(opt => (
                    <button key={opt.id} type="button" onClick={() => setShipping(opt)}
                      ...
                      <div className="flex justify-between items-center">
                        <div><span className="text-charcoal text-sm font-light">{opt.labelAr}</span><span className="text-stone text-xs font-light ml-2">({opt.estimate})</span></div>
                        <span className="text-charcoal text-sm font-light">{opt.price === 0 ? 'مجاناً' : `EGP ${opt.price}`}</span>
                      </div>
                    </button>
                  ))}</div>
                </div>
