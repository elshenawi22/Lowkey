...
export default function CheckoutDrawer({ isOpen, onClose, onBack }: CheckoutDrawerProps) {
  ...
  const applyDiscount = async () => {
    if (!discountCode || couponLoading) return;
  ...
  const handleBlur = (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => {
    ...
    if (name === 'name' && (!form.name.trim() || form.name.trim().length < 3))
      e2.name = 'الاسم مطلوب (3 حروف على الأقل)';
    if (name === 'phone') {
      const ph = checkPhone(form.phone);
      if (!form.phone.trim()) e2.phone = 'رقم الموبايل مطلوب';
      else if (!ph.valid) e2.phone = ph.error;
    }
    if (name === 'email' && !checkEmail(form.email))
      e2.email = 'إيميل غير صحيح';
    if (name === 'city' && !form.city.trim())
      e2.city = 'المدينة مطلوبة';
    if (name === 'address' && (!form.address.trim() || form.address.trim().length < 5))
      e2.address = 'العنوان مطلوب';
  ...
  return (
    <>
      ...
      <aside className="fixed top-0 right-0 z-[90] h-full w-full max-w-md bg-cream overflow-y-auto">
        {/* Loading overlay */}
        {status === 'loading' && (
          <div className="absolute inset-0 z-10 bg-cream/80 flex flex-col items-center justify-center gap-4" aria-live="polite">
            <div className="w-6 h-6 border border-navy border-t-transparent rounded-full animate-spin" />
            <p className="text-stone text-[0.6rem] tracking-[0.3em] uppercase font-light">جاري الإرسال</p>
          </div>
        )}
        <div className="p-8" ref={panelRef}>
          ...
          {status === 'success' ? (
            ...
            <>
              ...
              <form onSubmit={handleSubmit} className="mt-6 space-y-4">
                <Fld label="الاسم بالكامل *" name="name" value={form.name} onChange={handleChange} onBlur={handleBlur} error={errors.name} />
                <Fld label="رقم الموبايل *" name="phone" value={form.phone} onChange={handleChange} onBlur={handleBlur} error={errors.phone} type="tel" placeholder="01012345678" hint="هيتحول تلقائي لـ 201012345678" />
                <Fld label="الإيميل *" name="email" value={form.email} onChange={handleChange} onBlur={handleBlur} error={errors.email} type="email" />
                <Fld label="المدينة *" name="city" value={form.city} onChange={handleChange} onBlur={handleBlur} error={errors.city} placeholder="القاهرة، بورسعيد..." />
                <Fld label="العنوان بالتفصيل *" name="address" value={form.address} onChange={handleChange} onBlur={handleBlur} error={errors.address} area placeholder="الشارع، المنطقة..." />
                ...
                <div>
                  ...
                  <div className="flex gap-2">
                    ...
                    <button
                      ...
                      disabled={couponLoading || !discountCode}
                      className="text-[0.6rem] tracking-[0.15em] uppercase text-navy border border-navy/30 px-4 py-2 hover:bg-navy hover:text-cream transition-all disabled:opacity-40 min-w-[3.5rem] flex items-center justify-center"
                    >
                      {couponLoading ? (
...
function Fld({ label, name, value, onChange, onBlur, error, type = 'text', placeholder, hint, area }: {
  label: string; name: string; value: string;
  onChange: (e: React.ChangeEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
  onBlur?: (e: React.FocusEvent<HTMLInputElement | HTMLTextAreaElement>) => void;
  error?: string; type?: string; placeholder?: string; hint?: string; area?: boolean;
}) {
  const cls = `w-full bg-transparent py-3 text-charcoal text-sm font-light placeholder:text-stone/40 focus:outline-none transition-colors ${
    error ? 'border-red-400' : 'border-sand focus:border-navy'
  }`;
  return (
    <div>
      <label className="text-stone text-[0.6rem] tracking-[0.2em] uppercase font-light block mb-1.5">
        {label}
      </label>
      {area ? (
        <textarea
          ...
          className={`${cls} border border-sand p-3 resize-none`}
        />
      ) : (
        <input
      ...
      {error && (
        <p className="text-red-500 text-[0.6rem] mt-1 font-light" role="alert">
          {error}
        </p>
      )}
    </div>
