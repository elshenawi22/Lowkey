            ...
            <div className="flex items-center gap-6 ml-auto md:ml-0">
              ...
              {wishlistCount > 0 && (
                <button
                  ...
                  onClick={() => {
                    const event = new CustomEvent('lk:open-wishlist');
                    window.dispatchEvent(event);
                  }}
                  className={`hidden md:flex type-label relative items-center gap-1 transition-opacity duration-300 ${
                    !isLight ? 'text-cream/70 hover:text-cream' : 'text-stone hover:text-charcoal'
                  }`}
                >
                  <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="0.8" aria-hidden="true">
                    <path d="M20.84 4.61a5.5 5.5 0 0 0-7.78 0L12 5.67l-1.06-1.06a5.5 5.5 0 0 0-7.78 7.78l1.06 1.06L12 21.23l7.78-7.78 1.06-1.06a5.5 5.5 0 0 0 0-7.78z" />
                  </svg>
                  <span className="text-[0.55rem] tracking-[0.15em]">{wishlistCount}</span>
                </button>
              )}
              {/* Bag */}
              <button
