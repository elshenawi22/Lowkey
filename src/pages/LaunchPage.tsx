...
export default function LaunchPage() {
  ...
  // cms.launch_image starts as '' (defaultContent) before Supabase responds.
  // cmsLoaded becomes true only once we have a real value (even an empty one
  // that was explicitly saved), so we never show hero.jpg as a placeholder.
  const [cmsLoaded, setCmsLoaded] = useState(false);
  useEffect(() => {
    // The CMS hook loads from localStorage first (fast), then Supabase.
    // We wait for the first non-default signal: if after 1.5s cms is still
    // the empty default, we accept hero.jpg as the intentional fallback.
    const timer = setTimeout(() => setCmsLoaded(true), 1500);
    return () => clearTimeout(timer);
  }, []);
  useEffect(() => {
    // If CMS data arrives early (localStorage cache hit), show it immediately.
    if (cms.launch_image) setCmsLoaded(true);
  }, [cms.launch_image]);

  const isImageUrl = cmsLoaded && !!cms.launch_image && !/\.(mp4|webm|mov|m4v)(\?|$)/i.test(cms.launch_image);
  const backgroundImage = !cmsLoaded
    ? null
    : isImageUrl
      ? cms.launch_image
      : '/images/hero.jpg';
  ...
  return (
    <main className="relative min-h-screen overflow-hidden">
      {/* Background — static image only, rendered only after CMS data arrives */}
