...
// Heavy pages — code-split from main bundle
const AdminPage    = lazy(() => import('./pages/AdminPage'));
const ArchivePage  = lazy(() => import('./pages/ArchivePage'));
const AboutPage    = lazy(() => import('./pages/AboutPage'));
const ReviewsPage  = lazy(() => import('./pages/ReviewsPage'));

// Minimal page-level suspense fallback — avoids layout shift
function PageLoader() {
  return (
    <div className="min-h-screen bg-cream flex items-center justify-center" aria-label="Loading">
      <div className="w-5 h-5 border border-navy/30 border-t-navy rounded-full animate-spin" />
    </div>
  );
}
...
function Routes() {
  ...
  if (segments.length === 0) {
    ...
    page = <Suspense fallback={<PageLoader />}><ArchivePage /></Suspense>;
  } else if (segments[0] === 'about') {
    page = <Suspense fallback={<PageLoader />}><AboutPage /></Suspense>;
  } else if (segments[0] === 'track') {
    page = <TrackPage />;
  } else if (segments[0] === 'policy') {
    page = <PolicyPage />;
  } else if (segments[0] === 'reviews') {
    page = <Suspense fallback={<PageLoader />}><ReviewsPage /></Suspense>;
  } else {
    page = <NotFoundState />;
  }
  ...
  return (
    <>
      ...
      <a
        href="#main-content"
        className="sr-only focus:not-sr-only focus:fixed focus:top-4 focus:left-4 focus:z-[999] focus:px-4 focus:py-2 focus:bg-navy focus:text-cream focus:text-xs focus:tracking-widest focus:uppercase"
      >
        Skip to content
      </a>
      <Navigation />
